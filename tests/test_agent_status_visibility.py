...
_PAGES = ("/", "/dispositivos", "/whitelist", "/logs", "/ajustes")
...
def test_ajustes_hides_restart_hint_when_live(logged_in_client, session):
    # Setup
    profiles.record_agent_heartbeat(session)
    # Action
...
def test_ajustes_shows_restart_hint_when_never_seen(logged_in_client, session):
    # Action
    response = logged_in_client.get("/ajustes")
    # Expected
    assert response.status_code == status.HTTP_200_OK
...
def test_ajustes_shows_restart_hint_when_stale(logged_in_client, session):
    # Setup
    profiles.record_agent_heartbeat(session)
    settings = profiles.get_settings(session)
    settings.agent_last_heartbeat_at = datetime.now(timezone.utc) - timedelta(seconds=profiles._HEARTBEAT_STALE_SECONDS + 1)
...
def test_ajustes_shows_precise_last_heartbeat_timestamp(logged_in_client, session):
    # Setup
    profiles.record_agent_heartbeat(session)
    settings = profiles.get_settings(session)
